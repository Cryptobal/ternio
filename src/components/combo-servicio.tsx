...
const PREGUNTA_SERVICIO = '¿Qué servicio necesitas?'

export function ComboServicio({
  servicios,
  onElegir,
  pregunta = PREGUNTA_SERVICIO,
  idPrefijo = 'combo-servicio',
  abrirAlMontar = false,
}: {
  servicios: ServicioCombo[]
  onElegir: (slug: string) => void
  /** Pregunta contextual (p. ej. "…para tu casa") tras elegir audiencia. */
  pregunta?: string
  idPrefijo?: string
  ...
  const filtrados = useMemo(
    () => ordenarServiciosPorNombre(filtrarServiciosPorTexto(servicios, query)),
    [servicios, query],
  )
  ...
  if (escritorio === null) {
    return (
      <div>
        <p className={CLASE_LEYENDA_NAVY}>{pregunta}</p>
