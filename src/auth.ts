 ...
 *  - Google, para el comprador (se le pide DESPUÉS de enviar el formulario).
 *  - Credentials, solo para el dueño (rol ADMIN), en la ruta oculta.
 *
...
export const { handlers, auth, signIn, signOut } = NextAuth({
  ...
  providers: [
    ...
    Credentials({
      ...
      async authorize(credenciales) {
        ...
        // Freno a la fuerza bruta por cuenta.
        const limite = consumirRateLimit(`login-admin:${email}`, 5, 5 * 60_000)
        if (!limite.permitido) return null

        const usuario = await prisma.user.findUnique({ where: { email } })
        // Solo el rol ADMIN puede entrar por Credentials; un comprador con
        // contraseña filtrada no abre el panel.
  ...
  callbacks: {
    ...
    async jwt({ token, user }) {
      // Al iniciar sesión guardamos el rol en el token para que el middleware
      // pueda decidir sin tocar la base de datos.
      if (user?.id) {
        token.sub = user.id
