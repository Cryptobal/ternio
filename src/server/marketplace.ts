...
import { avisarLeadTomado } from '@/server/avisos-email'
  ...
  try {
    const resultado = await prisma.$transaction(
      async (tx) => {
        ...
        return { mensaje: 'Ya es tuyo. Ahí está el contacto.', compraId: compra.id, leadId: lead.id }
      },
      { isolationLevel: Prisma.TransactionIsolationLevel.Serializable },
    )

    if (resultado.compraId && resultado.leadId) {
      await avisarLeadTomado(resultado.leadId, resultado.compraId)
    }
