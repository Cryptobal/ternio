...
const SLUGS_PRIMEROS = [
  ...
  'supervisor-de-seguridad-cuando-pedirlo-en-chile',
  ...
  'control-vehicular-bodega-planta-seguridad-chile',
  ...
  'alarma-monitoreada-o-guardia-empresa-chile',
  'escalamiento-incidentes-seguridad-empresa-chile',
] as const
...
describe('posts publicados', () => {
  ...
  it('son 27, más nuevo primero, slugs pedidos', () => {
    ...
    expect(posts[0]?.date).toBe('2026-09-18')
    expect(posts.at(-1)?.date).toBe('2026-08-13')
    expect(new Set(posts.map((post) => post.date)).size).toBe(26)
  })
  ...
  it('cada post renderiza HTML, metadescripción propia y CTA al rubro', () => {
    ...
    const destinos: Record<(typeof SLUGS_PRIMEROS)[number], string> = {
      ...
      'supervisor-de-seguridad-cuando-pedirlo-en-chile': '/seguridad#cotizar',
      ...
      'control-vehicular-bodega-planta-seguridad-chile': '/seguridad#cotizar',
      ...
      'alarma-monitoreada-o-guardia-empresa-chile': '/seguridad#cotizar',
      'escalamiento-incidentes-seguridad-empresa-chile': '/seguridad#cotizar',
    }
    const titles = posts.map((post) => post.title)
    const descriptions = posts.map((post) => post.description)
    expect(new Set(titles).size).toBe(27)
    expect(new Set(descriptions).size).toBe(27)
